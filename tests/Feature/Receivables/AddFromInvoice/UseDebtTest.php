...
namespace Tests\Feature\Receivables\AddFromInvoice;
...
class UseDebtTest extends TestCase
{
    ...
    public function test_use_pending_receivable_with_debt_greater_than_new_invoice_amount_for_compensations()  //@codingStandardsIgnoreLine
    {
        ...
        $job = app(CreateFromInvoice::class, ['data' => ['id' => 743]]);
        ...
        $negativeInvoice = Invoice::find(268);
    ...
    {
        $negativeReceivable = Receivable::find(268);
        $negativeReceivable->import = -300;
        $negativeReceivable->total_amount_paid = 0;
        $negativeReceivable->difference = -300;
        $negativeReceivable->status = 'PENDING';
        $negativeReceivable->save();

        Receivable::query()->where('contract_id', 1)
            ->where('source', 'Electricidad')
            ->where('id', '>', 268)->delete();

        ReceivableHistories::where('receivable_id', 268)
            ->delete();

        $negativeInvoice = Invoice::find(268);
        $negativeInvoice->status = 'PENDING';
        $negativeInvoice->total_amount = -300;
        $negativeInvoice->save();

        Invoice::query()->where('contract_id', 1)
            ->where('id', '>', 43547)->delete();
